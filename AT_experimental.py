...
devie = torch.device(1 if torch.cuda.is_available() else 'cpu')
# Configuration
...
pose_cfg = 'mmaction2/demo/demo_configs/td-hm_hrnet-w32_8xb64-210e_coco-256x192_infer.py'
...
def process_and_display(batchFrames, fps):
    ...
    for frame_idx, (frame, result) in enumerate(zip(batchFrames, results)):
        ...
        pose_data_samples: List[PoseDataSample] \
            = inference_topdown(pose_model, frame, detXYXY, bbox_format='xyxy')
        ...
        pose_datasample.append(pose_data_sample)
...
def spte_acre(args):
    ...
    framesDet, human_detections, targetFrameList, pose_results, pose_datasample = [], [], [], [], []
    start = time.time()
    while True:
        # Detection
        ret, frame = cap.read()
    ...
    # print("what inside pose resutls: \n", pose_results[-1])
    # print("what inside pose resutls type: \n", type(pose_results[-1]))
    # print("pose results type: ", type(pose_results))
    ...
    print("how long frame_path: ", len(frame_paths))
    print("how long frames: ", len(frames))
